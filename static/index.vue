        ...
        <q-form @submit="sendFormData" class="q-gutter-md">
          ...
          <div v-if="formDialog.data.id" class="q-mt-md">
            ...
            <q-toggle
              ...
              color="primary"
              :label="formDialog.data.auth_enabled ? 'Identity Login ENABLED' : 'Identity Login DISABLED'"
              dense
              class="q-mb-sm"
            ><q-tooltip>When disabled the device cannot authenticate wallets or NFC cards — all /auth endpoints refuse.</q-tooltip></q-toggle>

            <!-- Expanded content — hidden when auth_enabled is false -->
            <div v-if="formDialog.data.auth_enabled">

              <!-- Teach mode + Teach-PIN side by side -->
              <div class="row q-col-gutter-md q-mb-sm q-mt-xs">
                <div class="col-auto">
                  <q-toggle
                    v-model="formDialog.data.touch_enabled"
                    color="primary"
                    :label="formDialog.data.touch_enabled ? 'Teach mode is ENABLED' : 'Teach mode is DISABLED'"
                    dense
                  ><q-tooltip>Gate for enrolling new identities. Auto-disabled after 3 wrong Teach-PINs; re-enable here.</q-tooltip></q-toggle>
                </div>
                <div class="col">
                  <q-input
                    filled dense
                  ...
                  ><q-tooltip>6-digit PIN entered on the device (6 taps + hold) to open teach mode. Verified server-side.</q-tooltip></q-input>
                </div>
              </div>

              <!-- ── LNURL-auth Identities ──────────────────────────────── -->
              <q-separator class="q-my-sm"></q-separator>
              <div class="row items-center q-mb-xs">
                <div class="col text-subtitle2">
                  LNURL-auth Identities <span class="text-grey text-weight-regular">(Lightning Wallets)</span>
                </div>
                <q-btn flat dense round size="sm" icon="refresh" color="primary" @click="getAuthKeys(formDialog.data.id)">
                  <q-tooltip>Reload identities</q-tooltip>
                </q-btn>
              </div>
              <div v-if="authKeys.length === 0" class="text-caption text-grey q-mb-sm">
                No wallets enrolled yet. Open teach mode on the device (6 taps + hold) and scan the register QR with a wallet.
              </div>
              <q-list v-else bordered separator dense class="q-mb-sm">
                <q-item v-for="ak in authKeys" :key="ak.id">
                  <q-item-section>
                    <q-input dense borderless v-model.trim="ak.label" placeholder="Label" @blur="updateAuthKey(ak)"></q-input>
                    <q-item-label caption>{{ shortKey(ak.pubkey) }} · {{ formatDate(ak.created_at) }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-toggle v-model="ak.enabled" color="primary" dense @update:model-value="updateAuthKey(ak)">
                      <q-tooltip>Enable / disable this wallet</q-tooltip>
                    </q-toggle>
                  </q-item-section>
                  <q-item-section side>
                    <q-btn flat dense size="xs" icon="cancel" color="pink" @click="deleteAuthKey(ak)">
              ...
              <!-- ── NFC Identities – NTAG424 ──────────────────────────── -->
              <q-separator class="q-my-sm"></q-separator>
              <div class="row items-center q-mb-xs">
                <div class="col">
                  <div class="text-subtitle2">
                    NFC Identities – NTAG424 <span class="text-grey text-weight-regular">(Bolt Card, Ring, etc.)</span>
                  </div>
                  <div class="text-caption text-grey">Note: Use with <a href="https://github.com/AxelHamburch/tagid_extension" target="_blank" class="text-grey">TagID🔐 Extension</a></div>
                </div>
                <q-btn flat dense round size="sm" icon="refresh" color="primary" @click="getNfcIdentities(formDialog.data.id)">
                  <q-tooltip>Reload NFC cards</q-tooltip>
                </q-btn>
              </div>
              <q-input
                filled dense
                ...
                class="q-mb-sm"
              ><q-tooltip>Base URL of the tagid_extension, e.g. https://my-lnbits.com/tagid</q-tooltip></q-input>
              <q-input
                filled dense
                ...
                class="q-mb-sm"
              ><q-tooltip>Invoice API key of the tagid wallet (used for server-to-server verify calls)</q-tooltip></q-input>
              <div v-if="nfcIdentities.length === 0" class="text-caption text-grey q-mb-sm">
              ...
              <q-list v-else bordered separator dense>
                <q-item v-for="nfc in nfcIdentities" :key="nfc.id">
                  <q-item-section>
                    <q-input dense borderless v-model.trim="nfc.label" placeholder="Label" @blur="updateNfcIdentity(nfc)"></q-input>
